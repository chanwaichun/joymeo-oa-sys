...
<script>
	 ...
	 * @event {Function} onSuccess 图片上传成功时触发
	 * @event {Function} on-change 图片上传后，无论成功或者失败都会触发
	 * @event {Function} onError 图片上传失败时触发
	...
	export default {
		...
		methods: {
					...
					.then(res => {
						...
						res.tempFiles.map((val, index) => {
							...
							if (val.size > maxSize) {
								...
								this.showToast(`请上传${maxSize/(1024*1024)}M以内的图片`);
							} else {
				...
				const task = uni.uploadFile({
					...
					success: res => {
						...
						if (![200, 201, 204].includes(res.statusCode)) {
							...
							this.lists[index].error = false;
							this.$emit('onSuccess', data, index, this.lists, this.index);
			...
			uploadError(index, err) {
				...
				this.lists[index].response = null;
				this.$emit('onError', err, index, this.lists, this.index);
			...
			handlerDeleteItem(index) {
				...
				this.$forceUpdate();
				this.$emit('onRemove', index, this.lists, this.index);
				this.showToast('移除成功');
			},
			// 用户通过ref手动的形式，移除一张图片
			remove(index) {
