<template>
	<Card class="summaryCard" titleWeight="normal" titleMargin="20" :paddingBottom="true" :card-title="cardTitle">
		<DashBoardTime :filter="filter" v-if="!isLoading&&showTime"></DashBoardTime>
			...
			<view class="summaryCard-content u-flex u-flex-wrap" v-if="summaryData">
				<template v-for="(item, index) in fieldData">
					<view class="content-ratio" v-if="index % 4 === 0" :key="index">
						<view v-show="showRatioName">{{ item.dayRatioKey ? "日" : "" }}</view>
						<view v-show="showRatioName">{{ item.weekRatioKey ? "周" : "" }}</view>
					</view>
...
<script>
	...
	const needFilter = ['getHisPurchaseAmountDataView', 'getOtcSalesDataView', 'getHisYesterdayData','getIncomeAmountData','getHisDataSummary'];
	export default {
		name: 'SummaryCard',
		components: {
			DashBoardTime,
		...
		mounted() {
			if (this.filter) {
				if (needFilter.includes(this.api)) {
					...
					this.getSummaryData(this.filter)
		...
		methods: {
			...
			getSummaryData(currentFilter) {
				...
				this.$requestData(apiConfig.dashBoard[this.api], filter).then((res) => {
					...
					// this.summaryData = {
					// 	"statDate": "2022-03-29",
					// 	"patientNum": 123213123,
					// 	"recipeNum": 123123123,
					// 	"logClinicNum": 12002020,
					// 	"visitVum": 12312321,
					// 	"clinicNumGrowthByDay": 0.11,
					// 	"activeClinicNumGrowthByDay": 0.11,
					// 	"averageRecipeNum": 123213,
					// 	"averageRecipeNumGrowthByDay": 0.111,
					// 	"activeClinicNum": 123123,
					// 	"warehouseSkuNum": 12312312312,
					// 	"visitNumGrowthByDay": 0.111,
					// 	"recipeNumGrowthByDay": 0.11,
					// 	"patientNumGrowthByDay": 0.11,
					// 	"warehouseSkuNumGrowthByDay": 0.111,
					// 	"periodDates": 12,
					// };
					// this.summaryData = {
					// 	"statDate": "2022-03-29",
					// 	"logClinicNum": 1232131230, //新增诊所
					// 	"cooperateClinicNum": 1232131230, //签约诊所
					// 	"visitNum": 1232131230, //接诊数
					// 	"recipeNum": 1232131230, //处方单
					// 	"activeClinicNum": 1232131230, //活跃诊所
					// 	"patientNum": 1232131230, //接诊人数
					// 	"warehouseSkuNum": 1232131230, //诊所商品数
					// 	"clinicNumGrowthByDay": 0.11,
					// 	"activeClinicNumGrowthByDay": 0.11,
