<template>
	<Card class="rankCard u-flex" titleWeight="normal" :titleMargin="titleMargin" :paddingBottom="true"
		...
		<view slot="titleIcon" v-if="showSort">
			<view class="time-desc" @click="setDesc">
		...
		<template v-if="!isLoading">
			<DashBoardTime :filter="propsFilter" v-if="showTime"></DashBoardTime>
			<view class="rankCard-header u-flex" :class="[showRank ? '':'hideRank']" v-if="showHeader">
			...
			<view class="rank" v-if="rankData.length > 0">
				...
				<view class="rank-item" v-for="(item,index) in rankData" :key="index">
					<view class="item-describe">
						<view class="describe-rank" v-if="showRank">{{rankValue ? item[rankValue]:index+1}}</view>
	...
	export default {
		...
		mounted() {
			if (this.propsFilter) {
				...
				this.getRankData(this.propsFilter)
			} else {
