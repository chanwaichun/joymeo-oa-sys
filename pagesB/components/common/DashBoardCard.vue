<template>
	<Card titleMargin="20" titleWeight="normal" class="dashBoardCard" :paddingBottom="true" :card-title="cardTitle">
		<view v-if="pageToDetail" slot="title-right" class="dashBoardCard-titleSlot" @click="navigateToDetail">查看明细
		</view>
		<view v-if="!isLoading">
			<DashBoardTime :filter="filter"></DashBoardTime>

			<view class="dashBoardCard-lastDayData" v-if="showLastDayValue && data.length > 0">
			...
			<view class="u-flex">
				<slot name="totalValue"></slot>
				<view v-if="growthSpeed">
					<NumberTag label="增速" :field="growthSpeed" :data="growthSpeedValue"></NumberTag>
				</view>
				<template v-if="drugTotal && drugData">
					<NumberTag label="总" field="drugsTotalSum" :data="drugData.drugsTotalSum">
					</NumberTag>
					<text class="totalValue-gap" style="margin:0 12rpx;">|</text>
					<NumberTag label="卓悦药品" field="joymeoDrugSum" :data="drugData.joymeoDrugSum">
					</NumberTag>
					<text class="totalValue-gap" style="margin:0 12rpx;">|</text>
					<NumberTag label="占比" field="clinicActiveRate"
						:data="drugData.drugsTotalSum ? drugData.joymeoDrugSum/drugData.drugsTotalSum : 0">
					</NumberTag>
				</template>
				<template v-if="otherNumberTag && dayNum">
					<text v-if="otherNumberTag === 'clinicActiveRate'" class="totalValue-gap" style="margin:0 12rpx;">|</text>
					<NumberTag v-if="otherNumberTag === 'clinicActiveRate'" label="平均活跃率" field="clinicActiveRate" :data="otherValue/dayNum">
					</NumberTag>
					<text v-if="otherNumberTag === 'totalClinicNum'" class="totalValue-gap" style="margin:0 12rpx;">|</text>
					<NumberTag v-if="otherNumberTag === 'totalClinicNum'" label="平均" field="totalClinicNum" :data="otherValue/dayNum">
					</NumberTag>
				</template>
			</view>
...
<script>
	...
	import util from '@/pagesB/utils/index.js'
	import DashBoardTime from './DashBoardTime.vue'
	import NumberTag from './NumberTag.vue'
	...
	export default {
		...
		data() {
			return {
				...
				fieldBigData,
				growthSpeedValue: null,
				clinicActiveRateSum: null,
				otherValue: null,
			}
		},
		watch: {
		...
		methods: {
			navigateToDetail() {
				...
				uni.navigateTo({
					url: `${url}${this.$util.paramsObjectToString({url,
					title,
					currentFilter,
					currentPage,
					pageFilter})}`
			...
			setCanvasId() {
				...
				return `${this.api.slice(0,10)||randomName}-${time}`
			},
			getData(filter) {

				this.isLoading = true
				this.dataLastValue = null;
				this.dataLastDate = null;
				this.growthSpeedValue=null;
				this.drugData = null
				...
				this.$requestData(apiConfig.dashBoard[this.api], filter).then(res => {
					...
					if (res.data.length > 0 && res.data[0]) {
						...
						// 用于HIS的增速
						this.growthSpeedValue = this.data[this.data.length - 1][this.growthSpeed]
						if (this.otherNumberTag) {
							this.otherValue = this.data[this.data.length - 1][this.otherNumberTag]
						}
						if (this.drugTotal) {
							this.drugData = this.data[this.data.length - 1]
						}
