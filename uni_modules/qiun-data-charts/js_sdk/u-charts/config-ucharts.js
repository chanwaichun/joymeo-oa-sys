...
const color = ['#4975C0', '#38D293', '#FAC858', '#EE6666', '#73C0DE', '#3CA272', '#FC8452', '#9A60B4', '#ea7ccc'];
...
const cfu = {
	...
	"formatter": {
		...
		"tooltipMix": function(item, category, index, opts) {
			// debugger;
			const {
				type
			} = item
			...
			const currenIndex = series.findIndex(item => {
				return item.type === type;
			})
			const keyType = fieldBigData.getFieldAttribute(series[currenIndex].key, 'type', true)
			return `${category} ${series[currenIndex].name}:${keyType === 'rate' ? `${series[currenIndex]['data'][index]}%` : series[currenIndex]['data'][index] }`
		},
